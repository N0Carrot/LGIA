...
def cw_ssim(img_batch, ref_batch, scales=5, skip_scales=None, K=1e-6, reduction="mean",data_range=1.0):
    """
    计算一个batch的SSIM（结构相似性）平均值和最大值。

    Args:
        img_batch (torch.Tensor): 预测图像，形状为 (B, C, H, W)
        ref_batch (torch.Tensor): 参考图像，形状为 (B, C, H, W)
        data_range (float): 图像像素值范围，默认为1.0
        reduction (str): 归约方式，默认为"mean"

    Returns:
        tuple: (ssim_mean, ssim_max)
            ssim_mean: SSIM的平均值
            ssim_max: SSIM的最大值
    """
    import torch
    ...
    def _ssim(img1, img2, data_range, window_size=11, window=None, size_average=True, K=(0.01, 0.03)):
        """单张图片的SSIM实现，支持batch"""
        C = img1.size(1)
        if window is None:
            # 创建高斯窗口
            ...
            window = _2D_window.expand(C, 1, window_size, window_size).contiguous().to(img1.device).type(img1.dtype)
    ...
    # 计算每个样本的ssim
    ssim_vals = _ssim(img_batch, ref_batch, data_range=data_range, size_average=True)
    ssim_mean = ssim_vals.mean().item()
    ssim_max = ssim_vals.max().item()
    return ssim_mean, ssim_max
...
def image_identifiability_precision(
    ...
    #############################
    # my implement
    true_labels = list(true_user_data["labels"])
    rec_labels = list(reconstructed_user_data["labels"])
    diff_labels = diff_elements(true_labels, rec_labels)
    for batch_idx, reconstruction in enumerate(reconstructed_user_data["data"]):
        rec_label = reconstructed_user_data["labels"][batch_idx]
        label_subset = [0]
        # Find the closest match to the reconstruction in all of the validation data:
        distances = dict(zip(scores, [[] for entry in scores]))
        idx_numb = 0
        for idx in label_subset:
            if rec_label in true_labels:
                comparable_data = true_user_data["data"][true_labels.index(rec_label)]
            elif idx_numb < len(diff_labels):
                comparable_data = true_user_data['data'][true_labels.index(diff_labels[idx_numb])]
                idx_numb += 1
            else:
                comparable_data = true_user_data['data'][batch_idx]
...
def diff_elements(true_list, rec_list):
    list3 = []
    for i in true_list:
