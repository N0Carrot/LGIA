...
class GussianSimilarity(GradientLoss):
    """Gradient matching based on the gaussian similarity of two gradient vectors."""
    def __init__(self, scale=1.0, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.task_regularization = task_regularization
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data): 
        return self._gussian_similarity(gradient_rec, gradient_data) * self.scale

    def __repr__(self): 
        return f"GussianSimilarity loss with scale={self.scale} and task reg={self.task_regularization}"

    @staticmethod
    @torch.jit.script
    def _gussian_similarity(gradient_rec: List[torch.Tensor], gradient_data: List[torch.Tensor]): 
        sigma = 2
        diff = gradient_rec[0].flatten(start_dim=1) - gradient_data[0].flatten(start_dim=1)
        squared_distance = torch.norm(diff, p=2, dim=1) ** 2
        gaussian_sim = torch.exp(-squared_distance / (2 * sigma**2))
        gaussian_loss = 1 - gaussian_sim.mean() 
...
class DynaLayerRandPickedCosineSimilarity(GradientLoss):
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        scalar_product, rec_norm, data_norm = 0.0, 0.0, 0.0
        # flag = False
        if self.iter < 2000: #STAGE 1 T1
            if (self.iter + 1) % self.peroid_Add10 == 0:
                self.start =self.start- 3 #p
                final1 =  self.start + 20 #L_min
                self.final = min(len_layer, final1) 
        else: #STAGE 2
             if (self.iter + 1) % self.peroid_Add10 == 0:
                 start1= self.start- 10 #q
                 self.start  = max(start1, 40) #L_max
                 self.final = len_layer 
        self.iter += 1
        ...
        cosine_similarity = 1 - scalar_product / rec_norm.sqrt() / data_norm.sqrt() #cosine_similarity
        gaussian_value = torch.exp(-0.5 * ((rec_norm)** 2)) #gaussian ker
        objective = cosine_similarity *7+gaussian_value *7
        return objective
